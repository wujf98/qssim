...
4. 回到最初的应用场景，对每一个待加入仓库的图包，可以考虑只生成每个图包中少量图像的指纹信息与仓库进行比较，这不仅可以缩短生成指纹的时间，也可缩短比较指纹的时间；如果你希望对待加入仓库的所有图像与仓库进行指纹比较，可以考虑使用其他批量处理工具(如格式工厂和ImageMagick等等)生成相比待加入仓库图像在尺寸上更小的副本，生成这个副本的指纹所需要的的时间开销会小得多；此外，你也可以考虑放弃分块，只对一整张图像生成指纹信息，也可以加快生成指纹与比较指纹的速度。
